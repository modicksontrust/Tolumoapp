...
import { Calendar, Clock, Play, Star, Video, X, Info } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-5">
          ...
          <div className="flex items-center justify-center gap-2">
            {[1, 2, 3, 4, 5].map(n => (
              <button key={n} onMouseEnter={() => setHover(n)} onMouseLeave={() => setHover(0)} onClick={() => setStars(n)} className="transition-transform hover:scale-110">
                <Star className={`h-8 w-8 transition-colors ${n <= (hover || stars) ? 'fill-amber-400 text-amber-400' : 'text-stone-300'}`} />
              </button>
            ))}
          </div>
          <div>
          ...
          <div className="flex gap-3">
            <button onClick={() => { if (stars > 0) onSubmit(session.id, stars, comment); }}
              disabled={stars === 0}
            ...
            <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-stone-200 text-foreground font-semibold text-sm hover:bg-stone-50 transition-colors">
              Cancel
            </button>
          </div>
        </div>
...
type PastSession = {
  ...
  rated: boolean;
  recordingUrl: string;
  recordingExpires: string; // "3 Jan 2026"
};
...
const PAST: PastSession[] = [
  {
    ...
    stayedFull: true,
    rated: false,
    recordingUrl: 'https://recordings.tolumo.ng/sep-powers-jul3',
  ...
  },
  {
    ...
    stayedFull: true,
    rated: false,
    recordingUrl: 'https://recordings.tolumo.ng/postal-rule-jun25',
    recordingExpires: '25 Dec 2025',
  },
  {
    id: 3,
    module: 'Constitutional Law',
    ...
    stayedFull: false,
    rated: false,
    recordingUrl: 'https://recordings.tolumo.ng/supremacy-jul10',
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          All personal tutorial sessions you have booked. Book more directly from any topic screen after submitting feedback.
        </p>
      </div>

      {/* Tabs */}
      <div className="flex gap-0 bg-stone-100 rounded-xl p-1 w-fit">
        {([['upcoming', 'Upcoming Sessions'], ['past', 'Past Tutorials & Recordings']] as const).map(([key, label]) => (
      ...
      {tab === 'upcoming' && (
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl px-5 py-3.5">
            <Video className="h-4 w-4 text-amber-600 shrink-0 mt-0.5" />
            <p className="text-sm text-amber-800 leading-relaxed">
              <span className="font-semibold">Sessions are recorded automatically.</span>{' '}
              No opt-in is required. Your recording will appear in Past Tutorials within 2 hours of the session ending and is retained for 6 months from the session date.
            </p>
          </div>
          ...
          {UPCOMING.length === 0 ? (
            <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Book a session directly from any topic screen after submitting your topic feedback.</p>
            </div>
          ) : (
            UPCOMING.map(s => (
              <div key={s.id} className="rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
                <div className="bg-[#1a4d35] px-6 py-5">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 mb-1">{s.module} · Personal Tutorial</p>
                ...
                <div className="bg-white px-6 py-5 space-y-4">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-5 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4 shrink-0" />{s.date} · {s.time}</span>
                      <span className="flex items-center gap-1.5"><Clock className="h-4 w-4 shrink-0" />{s.duration}</span>
                    </div>
                  ...
                  <div className="rounded-xl bg-amber-50 border border-amber-100 px-5 py-3.5 space-y-0.5">
                    <p className="text-sm text-foreground">
                      ...
                      <a href={s.videoLink} className="text-primary underline underline-offset-2 hover:text-primary/80 transition-colors break-all">{s.videoLink}</a>
                    </p>
                    <p className="text-xs text-amber-700">Recording will be available in Past Tutorials within 2 hours after the session ends.</p>
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3 bg-stone-100 border border-stone-200 rounded-xl px-5 py-3.5">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              <span className="font-semibold text-foreground">Recording retention policy:</span>{' '}
              Every session is recorded automatically. Recordings remain available for <strong>6 months from the session date</strong>, after which they are automatically and permanently deleted.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm px-6 py-4 flex items-center gap-6 flex-wrap">
            ...
            <div className="flex items-center gap-4 text-xs ml-auto">
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-green-500 inline-block" />{onTime} on time / early</span>
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-red-500 inline-block" />{late} late</span>
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-stone-500 inline-block" />{stayed} stayed full</span>
            </div>
          </div>
          ...
          {PAST.map(s => (
            <div key={s.id} className="rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
              <div className="bg-[#1a4d35] px-6 py-5 relative">
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 mb-1">{s.module} · Past Tutorial</p>
                <h2 className="font-serif font-bold text-white text-lg mb-3">{s.title}</h2>
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 rounded-full bg-white/20 flex items-center justify-center shrink-0">
                    <span className="text-white text-[10px] font-bold">{s.tutorInitials}</span>
                ...
                <div className="absolute top-5 right-5"><Badge session={s} /></div>
              </div>
              ...
              <div className="bg-white px-6 py-5 space-y-4">
                ...
                {/* Recording block */}
                <div className="rounded-xl bg-stone-50 border border-stone-100 px-5 py-3.5 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                      <Video className="h-3.5 w-3.5 text-muted-foreground" /> Session Recording
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Available until {s.recordingExpires} · deleted permanently after 6 months
                    </p>
                  </div>
      ...
      {ratingSession && (
        <RatingModal
          ...
          onSubmit={(id, stars) => { setRatings(r => ({ ...r, [id]: stars })); setRatingSession(null); }}
